...
class MenuDatabase():
    ...
    def insert(self, product_name, cost, shortcuts, color, shape, position):
        with self.connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"INSERT INTO {self.table} (product_name, cost, shortcuts, color, shape, position) VALUES (?, ?, ?, ?, ?, ?)",
                (product_name, cost, shortcuts, color, shape, position)
            )

    def fetch_all(self):
        with self.connect() as conn:
            cursor = conn.cursor()
            cursor.execute(self._base_select_query())
            return cursor.fetchall()
...
# New MenuDatabase implementation with dedicated schema for menu items.
# This redefinition intentionally overrides the previous class definition.
class MenuDatabase():
    def __init__(self, database, table):
        if not database.endswith(".db"):
            raise ValueError("Database name must end with .db")
